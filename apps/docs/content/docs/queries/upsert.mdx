---
title: upsert
description: Create a record if it doesn't exist, or update it if it does — with conditional field logic, unset, and nested relations.
---
...
| `unset` | `UnsetInput` | No | Fields to remove on update (set to NONE). Ignored on create path |
...
See [`updateMany` — Unsetting Fields](/queries/update-many#unsetting-fields) for full details on nested object/tuple unset syntax and `SafeUnset` cross-exclusion with `update` data.
...
Upsert queries work with [`$transaction`](/transactions) for atomic batch execution:
...
On the create path, the same auto-population rules as [`create`](/queries/create) apply:
