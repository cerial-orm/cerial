...
@Injectable()
export class CerialService implements OnModuleInit, OnModuleDestroy {
  public readonly client = new CerialClient();
...
Inject `CerialService` into your feature services to access the typed client:
...
export class UsersService {
  ...
  async findAll() {
    return this.cerial.db.User.findMany();
  }

  async findOne(id: string) {
    return this.cerial.db.User.findUnique({ where: { id } });
  }

  async create(data: { email: string; name: string }) {
    return this.cerial.db.User.create({ data });
  }
}
```
...
@Injectable()
export class AuthCerialService implements OnModuleInit, OnModuleDestroy {
  public readonly client = new AuthCerialClient();
...
export class PostsService {
  ...
  async findAll() {
    return this.cms.db.Post.findMany();
  }
}
```
...
export class OrdersService {
  ...
  async createOrder(userId: string, items: { productId: string; quantity: number }[]) {
    return this.cerial.client.$transaction(async (tx) => {
