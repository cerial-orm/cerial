...
Self-referential models also use `@key` when they have multiple relations to themselves. See [Self-Referential Relations](/cerial/relations/self-referential/) for patterns like tree hierarchies, mentorship, and following systems that combine `@key` with self-referencing.
