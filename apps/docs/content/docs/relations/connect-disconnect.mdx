...
All connect, disconnect, and set operations work inside [`$transaction`](/transactions). When used within a transaction, the operations are covered by the same atomic boundary:
