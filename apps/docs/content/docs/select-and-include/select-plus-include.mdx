...
You can use `select` and `include` together in the same query. The return type is the intersection of selected fields and included relations — you get only the scalar fields you asked for, plus the loaded relations.
