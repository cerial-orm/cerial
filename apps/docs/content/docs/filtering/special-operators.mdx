...
description: Filter with isNull, isNone, isDefined, and between — null checks, absence checks, and inclusive range queries.
---

Special operators handle null checks, absence checks, and range queries. The available operators depend on the field's schema modifiers (`?` and `@nullable`).

## Operator Availability

| Operator | Available on | Checks for |
| --- | --- | --- |
| `isNull` | `@nullable` fields | Whether field is `null` |
| `isNone` | `?` (optional) fields | Whether field is absent (NONE/undefined) |
| `isDefined` | `?` (optional) fields | Inverse of `isNone` — field is present |
