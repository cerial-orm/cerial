...
| --- | --- | --- | --- | --- | --- |
| [`@default(value)`](/cerial/schema/decorators/defaults/default/) | Yes | Yes (when absent) | No | Yes | Any |
| `@defaultAlways(value)` | Yes | Yes (when absent) | Yes (when absent) | Yes | Any |
| [`@updatedAt`](/cerial/schema/decorators/datetime/updated-at/) | Yes | Yes (`time::now()`) | Yes (`time::now()`) | Yes | `Date` only |
| [`@createdAt`](/cerial/schema/decorators/datetime/created-at/) | Yes | Yes (`time::now()`) | No | Yes | `Date` only |
