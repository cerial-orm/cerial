...
On the PK side, `@model()` is paired with [`@field()`](/schema/decorators/relations/field/) to form a complete forward relation:
...
- Every `Relation` with [`@field()`](/schema/decorators/relations/field/) must also have `@model()`.
...
- [`@field`](/schema/decorators/relations/field/) — specifies which Record field stores the FK
