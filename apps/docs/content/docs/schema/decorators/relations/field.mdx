...
Every forward relation (PK side) requires `@field()` alongside [`@model()`](/cerial/schema/decorators/relations/model/). It connects the virtual `Relation` field to the actual `Record` field that stores the FK value in the database.
...
- Every `Relation` with `@field()` must also have [`@model()`](/cerial/schema/decorators/relations/model/).
...
- [`@model`](/cerial/schema/decorators/relations/model/) — declares the target model of the relation
