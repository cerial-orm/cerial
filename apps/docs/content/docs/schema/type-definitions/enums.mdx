...
- All values are **strings only** — if you need numbers, booleans, or mixed types, use a [Literal](/cerial/schema/type-definitions/literals/) instead
...
:::note
Non-enum [Literals](/cerial/schema/type-definitions/literals/) are excluded from `orderBy` because they can contain mixed types where ordering is ambiguous. Enums, being string-only, always have well-defined ordering.
:::
...
Duplicate values are automatically deduplicated. You can also use pick and omit syntax to selectively inherit values. See the [Extends](/cerial/schema/extends/) page for full details.
...
See [Enums vs Literals](/cerial/schema/type-definitions/enums-vs-literals/) for a detailed comparison.
