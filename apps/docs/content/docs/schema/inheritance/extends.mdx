...
Models can extend [abstract models](/cerial/schema/inheritance/abstract/) to inherit fields. The child gets its own table, its own generated types, and its own client accessor. Both concrete and abstract models can **only** extend abstract models — extending a concrete model is not allowed.
...
Composite directives (`@@index`, `@@unique`) belong to the model that declares them — they are **not** passed down to children. If a child needs the same index, it must declare its own:
...
abstract model BaseEntity {
  ...
  name String
  email Email
  @@unique([name, email])  # this directive stays on BaseEntity only
}

model User extends BaseEntity {
  age Int?
  @@unique([name, email])  # must be redeclared if User needs it too
}
```
...
If the parent marks a field as [`!!private`](/cerial/schema/inheritance/private/), it cannot be overridden — the child can still inherit it, and can omit it with pick/omit syntax, but redefining it in the child body is an error.
