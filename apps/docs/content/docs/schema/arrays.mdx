...
:::note
`Relation[]` also uses the `[]` syntax, but it represents relation **cardinality** (one-to-many or many-to-many), not a stored array field. See [Field Types](/schema/field-types/) for details on the `Relation` type.
:::
