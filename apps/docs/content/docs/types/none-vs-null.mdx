---
title: NONE vs null
description: How SurrealDB's three-state field model (value, NONE, null) maps to TypeScript types, query operators, and update behavior.
---
...
See the [special operators](/filtering/special-operators) page for more details.
...
The `unset` parameter also supports nested object fields and tuple elements. See the [updateMany](/queries/update-many), [updateUnique](/queries/update-unique), and [upsert](/queries/upsert) docs for details.
...
`@default(null)` requires [`@nullable`](/schema/decorators/nullable) on the field. It converts omitted values to `null` on create:
