...
description: How Cerial computes return types based on select and include options using GetModelPayload, ResolveFieldSelect, and ApplyObjectSelect.
---
...
// Type: User & { posts: Post[] }
// The where/orderBy/limit filter at runtime but don't change the type
```
...
When using [`$transaction`](/transactions), each query's return type is preserved at its position in the result array:
...
TypeScript knows the exact type at each position. It's not a generic `unknown[]` but a precise typed tuple.
