...
| `fieldGroupBlankLines` | `'collapse'` \| `'single'` \| `'honor'` | `'collapse'` | Strip blank lines between fields / one blank line between groups / preserve original |
...
- **`'collapse'`** (default) ... all blank lines between fields are removed.
- **`'single'`** ... exactly one blank line between field groups. Consecutive blank lines collapse to one.
...
With `'single'`, blank lines from your source mark group boundaries:
...
- **`true`** ... trailing comma on the last element in tuple, enum, and literal blocks.
- **`false`** (default) ... no trailing comma.
