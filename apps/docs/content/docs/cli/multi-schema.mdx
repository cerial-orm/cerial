---
title: Multi-Schema
description: Split your data layer into multiple independent schemas, each generating its own typed client.
---
...
Run [`cerial init`](/cli/init) to auto-detect your schema folders and generate a config:
...
This filters the config to the named schema and generates only that client. The other schemas stay untouched.
...
Combine `-n` with `--watch` to watch just one schema:
...
Different schemas can connect to different databases. Set a root [connection](/connection) for shared defaults and override per-schema:
...
**Multiple roots**: Cerial detects them but requires a config file to know how to name and output each schema. It'll display an error pointing you to [`cerial init`](/cli/init).
...
For the full filtering reference, cascade priority, and gotchas, see [Path Filtering](/cli/filtering).
