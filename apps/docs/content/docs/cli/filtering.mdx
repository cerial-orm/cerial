...
description: Control which .cerial files are processed during generation with ignore, exclude, include, and .cerialignore.
---
...
:::note
`include` only overrides exclusions. Without `exclude` or a `.cerialignore` file, `include` has no effect. It's not a standalone whitelist filter.
:::
...
The `cms` schema excludes everything under `internal/` except `internal/public-api.cerial`, which is rescued by `include`.

:::warning
Don't use `ignore` when you want to rescue specific files. `ignore` is absolute, so `include` can't override it. Use `exclude` + `include` instead.
:::
...
Filtering is config-only. There are no `--ignore`, `--exclude`, or `--include` CLI flags. All filter rules live in your [config file](/cli/configuration) or `.cerialignore` files.

## `cerial init`

The [`cerial init`](/cli/init) command offers to create a `.cerialignore` file with sensible defaults when scaffolding your project config.
