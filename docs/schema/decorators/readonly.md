...
## @readonly on Record Fields (Foreign Keys)
...
await db.User.updateUnique({
  ...
  // address: { city: 'NYC' }  // Type error — 'city' excluded from Partial
});
```

When an object has `@readonly` sub-fields, the update type uses `Partial<Omit<ObjectInput, 'readonlyField'>>` to exclude those fields from merge updates.

## @readonly on Array Fields
...
`@readonly` adds the `READONLY` keyword to the field's `DEFINE FIELD` statement:

```sql
DEFINE FIELD slug ON article TYPE string READONLY;
DEFINE FIELD createdBy ON article TYPE option<string | null> DEFAULT 'system' READONLY;
```

For object sub-fields:

```sql
DEFINE FIELD address.city ON user TYPE string READONLY;
```

SurrealDB rejects any `UPDATE` that attempts to change a `READONLY` field with the error:

```
Found changed value for field `slug`, with record `article:1`, but field is readonly
```

Passing the same value as already stored is allowed (SurrealDB detects no change).
