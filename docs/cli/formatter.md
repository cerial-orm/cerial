...
Without `-s`, the formatter uses the same schema discovery as `cerial generate` (config file, convention markers, or fallback directories).
...
All formatting options go under the `format` key in your config. Every option has a sensible default, so you can skip this section entirely if the defaults work for you.
...
- [**generate**](./generate) - Run generation with formatting via `--format`
