...
describe('E2E Upsert Operations', () => {
  ...
  describe('unique field upsert — create path', () => {
    ...
    test('sets all create fields when creating', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'New User', email: 'new@test.com', isActive: true, age: 30 },
    ...
    test('update-only fields are absent (NONE) when creating', async () => {
      ...
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Created', email: 'create-noupdate@test.com', isActive: true },
  ...
  describe('unique field upsert — update path', () => {
    test('updates an existing record when match exists', async () => {
      ...
      const result = await client.db.User.upsert({
        where: { email: 'john@test.com' },
        create: { name: 'Should Not See', email: 'john@test.com', isActive: false },
        update: { name: 'John Updated' },
      });
    ...
    test('preserves create-only fields on update (existing values kept)', async () => {
      ...
      const result = await client.db.User.upsert({
        where: { email: 'john@test.com' },
        create: { name: 'New', email: 'john@test.com', isActive: true, age: 99 },
        update: { name: 'Updated' },
      });
  ...
  describe('create-only (no update)', () => {
    test('creates when record does not exist', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Create Only', email: 'createonly@test.com', isActive: true },
      });
    ...
    test('returns existing record unchanged when it already exists', async () => {
      await client.db.User.create({
        data: { name: 'Original', email: 'existing@test.com', isActive: true, age: 42 },
      });

      const result = await client.db.User.upsert({
        where: { email: 'existing@test.com' },
        create: { name: 'Should Not See', email: 'existing@test.com', isActive: false },
      });
  ...
  describe('ID-based upsert — create path', () => {
    ...
    test('sets all create fields when creating by ID', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Full Create', email: 'idfull@test.com', isActive: true, age: 28 },
        update: { name: 'Nope' },
      });
  ...
  describe('ID-based upsert — update path', () => {
    test('updates existing record with matching ID', async () => {
      await client.db.User.create({
        data: { id: 'user:testid2', name: 'Original', email: 'original@test.com', isActive: true },
      });

      const result = await client.db.User.upsert({
        where: { id: 'user:testid2' },
        create: { name: 'Should Not See', email: 'nope@test.com', isActive: false },
        update: { name: 'Updated' },
      });
    ...
    test('preserves non-updated fields on update path by ID', async () => {
      await client.db.User.create({
        data: { id: 'user:preserve', name: 'Original', email: 'preserve@test.com', isActive: true, age: 33 },
      });
  ...
  describe('ID-based create-only', () => {
    test('creates when record does not exist by ID', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'ID Create Only', email: 'idcreateonly@test.com', isActive: true },
      });
    ...
    test('returns existing record unchanged when it exists by ID', async () => {
      await client.db.User.create({
        data: { id: 'user:idexists', name: 'Existing', email: 'idexists@test.com', isActive: true },
      });
  ...
  describe('return options — WHERE-based (unique)', () => {
    ...
    test('return before returns null for new records (WHERE-based)', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Before Test', email: 'before@test.com', isActive: true },
        update: { name: 'Updated' },
        return: 'before',
      });
  ...
  describe('return options — ID-based', () => {
    test('return before returns null for new record (ID-based)', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Before ID', email: 'beforeid@test.com', isActive: true },
        update: { name: 'Updated' },
        return: 'before',
      });
    ...
    test('return before returns previous state for existing record (ID-based)', async () => {
      await client.db.User.create({
        data: { id: 'user:beforeid2', name: 'Original', email: 'beforeid2@test.com', isActive: true },
      });
    ...
    test('return true returns boolean for ID-based update', async () => {
      await client.db.User.create({
        data: { id: 'user:boolidexist', name: 'Existing', email: 'boolidexist@test.com', isActive: true },
      });
    ...
    test('default return for ID-based create returns the record', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'ID Default', email: 'iddefault@test.com', isActive: true },
        update: { name: 'Updated' },
      });
  ...
  describe('select', () => {
    test('returns only selected fields on create path', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Select Test', email: 'select@test.com', isActive: true, age: 25 },
        update: { name: 'Updated' },
    ...
    test('returns only selected fields for ID-based', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Select ID', email: 'selectid@test.com', isActive: true },
        update: { name: 'Updated' },
        select: { id: true, name: true },
      });
  ...
  describe('include', () => {
    test('includes related records in upsert result (update path)', async () => {
      const user = await client.db.User.create({
        data: { name: 'With Profile', email: 'profile@test.com', isActive: true },
      });

      const profile = await client.db.Profile.create({ data: { bio: 'Bio', userId: user.id } });
  ...
  describe('non-unique where (array return)', () => {
    test('returns array for non-unique where with matching records', async () => {
      await client.db.User.create({ data: { name: 'Alice', email: 'alice@test.com', isActive: true } });
      await client.db.User.create({ data: { name: 'Bob', email: 'bob@test.com', isActive: true } });
    ...
    test('non-unique where with return before returns array', async () => {
      await client.db.User.create({ data: { name: 'Alice', email: 'alice2@test.com', isActive: true } });
    ...
    test('non-unique where with return true returns boolean', async () => {
      await client.db.User.create({ data: { name: 'Alice', email: 'alice3@test.com', isActive: true } });
    ...
    test('non-unique where with select returns selected fields', async () => {
      await client.db.User.create({ data: { name: 'Alice', email: 'alice4@test.com', isActive: true } });
    ...
    test('works with $transaction for multiple upserts', async () => {
      const [user1, user2] = await client.$transaction([
        client.db.User.upsert({
          ...
          create: { name: 'Txn User 1', email: 'txn1@test.com', isActive: true },
          update: { name: 'Updated 1' },
        }),
        client.db.User.upsert({
          where: { email: 'txn2@test.com' },
          create: { name: 'Txn User 2', email: 'txn2@test.com', isActive: true },
  ...
  describe('auto-populated fields', () => {
    ...
    test('array fields default to empty array on create path', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'Array User', email: 'arrays@test.com', isActive: true },
        update: { name: 'Updated' },
      });
  ...
  describe('CerialId mapping', () => {
    test('returned id is a CerialId instance', async () => {
      const result = await client.db.User.upsert({
        ...
        create: { name: 'CerialId Test', email: 'cerialid@test.com', isActive: true },
        update: { name: 'Updated' },
      });
